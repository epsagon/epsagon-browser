...
class EpsagonExporter extends CollectorTraceExporter {
  ...
  convert(spans) {
    try{
      let convertedSpans = super.convert(spans)
      let spansList = convertedSpans.resourceSpans[0].instrumentationLibrarySpans
      let rootSpan = {
      ...
      for (let spanIndex in spansList) {
  
        let spanSubList = spansList[spanIndex].spans
  
        for(let spanSubIndex in spanSubList){
          ...
          let httpHost = spanAttributes.filter((attr) => { return attr.key === 'http.host' })
          let userInteraction = spanAttributes.filter((attr) => { return attr.value.stringValue === "user-interaction" })
          let documentLoad = spanAttributes.filter((attr) => { return attr.value.stringValue === "document-load" })
          let reactUpdates = spanAttributes.filter((attr) => { return attr.key === 'react_component_name' })
      ...
      if (rootSpan.eps.remove && rootSpan.doc.parent === rootSpan.eps.spanId) {
        let docLoad = spansList[rootSpan.doc['position']].spans[0]
  ...
  handleErrors(errSpan, spansList, rootSpan) {
    if(rootSpan.rootType === rootType.REDIR || rootSpan.rootType === rootType.DOC){
      ...
      let rootSubList = spansList[rootSpan[type]['position']].spans
      let rootSubPos = rootSpan[type]['subPosition']

      //errors get converted from their own spans to an event on the root span
      let s = new Set(errSpan.messages);
      ...
      spansList[rootSpan['doc']['position']].spans = spansList[rootSpan['doc']['position']].spans.filter((span)=>{return span.name != 'error'})   
    }else{
      ...
      spansList[rootSpan['doc']['position']].spans.map((span)=>{
        if(!spanErrs.includes(span.attributes[0].value.stringValue)){
          let errAttr = span.attributes.filter((attr)=> {return attr.key === 'message'})
          spanErrs.push(errAttr[0].value.stringValue)
          let attributesLength = this.addFinalGenericSpanAttrs(span.attributes, span.attributes.length, span);
  ...
  addFinalGenericSpanAttrs(spanAttributes, attributesLength, span) {
    ...
    let httpUA = spanAttributes.filter((attr) => { return attr.key === 'http.user_agent'; });
    ...
    span.attributes = spanAttributes.filter((attr) => { return attr.key != 'http.response_content_length_eps'; });
  ...
  formatUserInteractionSpan(spanAttributes, attributesLength) {
    ...
    attributesLength++;
    let eventType = spanAttributes.filter((attr) => { return attr.key === ('event_type'); });
  ...
  formatHttpRequestSpan(span, httpHost, spanAttributes, attributesLength) {
    ...
    let httpContentLength = spanAttributes.filter((attr) => { return attr.key === 'http.response_content_length'; });
    let epsHttpContentLength = spanAttributes.filter((attr) => { return attr.key === 'http.response_content_length_eps'; });
    if (epsHttpContentLength.length > 0) {
      httpContentLength[0]["value"]["intValue"] = epsHttpContentLength[0]["value"]["intValue"];
    }

    let httpUrlAttr = spanAttributes.filter((attr) => { return attr.key === 'http.url'; });
    let httpUrl = httpUrlAttr[0]["value"]["stringValue"];
  ...
  parseURL(httpUrl, span, spanAttributes, attributesLength) {
    ...
    if (httpUrl.indexOf('?') < 0 && httpUrl.indexOf(';') < 0) {
      let path = httpUrl.substring(httpUrl.indexOf(span.name) + span.name.length);
      spanAttributes[attributesLength] = { key: 'http.request.path', value: { stringValue: path } };
      attributesLength++;
    }
    if (httpUrl.indexOf('?') > 0) {
      let path = httpUrl.substring(httpUrl.indexOf(span.name) + span.name.length, httpUrl.indexOf('?'));
      spanAttributes[attributesLength] = { key: 'http.request.path', value: { stringValue: path } };
      let query = httpUrl.substring(httpUrl.indexOf('?'));
    ...
    if (httpUrl.indexOf(';') > 0) {
      let path = httpUrl.substring(httpUrl.indexOf(span.name) + span.name.length, httpUrl.indexOf(';'));
      spanAttributes[attributesLength] = { key: 'http.request.path', value: { stringValue: path } };
      let params = httpUrl.substring(httpUrl.indexOf(';'));
  ...
  addResourceAttrs(convertedSpans) {
    ...
    let appName = this.config.serviceName;
    ...
    convertedSpans.resourceSpans[0].resource.attributes = convertedSpans.resourceSpans[0].resource.attributes.filter((attr) => {
